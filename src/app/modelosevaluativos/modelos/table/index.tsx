...
export default function ModeloEvaluativoTable({
	data,
}: ModelosEvaluativosTableProps) {
	...
	return (
		<section className=''>
			<DataTable
				columns={modelosEvaluativosColumns}
...
function UpdateModeloEvaluativoModal({
	modelosEvaluativos,
}: {
	modelosEvaluativos: ModelosEvaluativoSchema[];
}) {
	...
	const selectedModeloEvaluativo = modelosEvaluativos.find(
		i => i.id === modeloEvaluativoParamsId,
	);
...
function DeactivateModeloEvaluativo({
	modelosEvaluativos,
}: {
	modelosEvaluativos: ModelosEvaluativoSchema[];
}) {
...
function CloneModeloEvaluativo({
	modelosEvaluativos,
}: {
	modelosEvaluativos: ModelosEvaluativoSchema[];
}) {
		...
		<section>
			<MutateModal
				...
				onSubmit={form.handleSubmit(data =>
					...
					mutation.mutate({
						data,
						currentModeloEvaluativo: selectedModeloEvaluativo,
					}),
				)}
...
function LogicaDeModeloEvaluativo({
	modelosEvaluativos,
}: {
	modelosEvaluativos: ModelosEvaluativoSchema[];
}) {
					...
					<div className='flex w-full flex-col items-start justify-start gap-4'>
						...
						<FormField
							...
							render={({ field }) => {
								return (
									<FormItem
										className='flex w-full items-center justify-between gap-4 space-y-0 rounded-2xl border-2 p-4'
										style={{
											boxShadow: "0 0 20px rgba(67, 84, 234, .7)",
										}}
									>
										...
										<FormControl>
											<Input {...field} type='custom-text-area' />
										</FormControl>
									</FormItem>
								);
							}}
