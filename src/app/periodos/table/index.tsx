...
export default function PeriodosLectivosTables() {
	return (
		<section>
			...
			<ActualizarSubperiodo periodos={MUPeriodos} />
		</section>
	);
}
...
function ActualizarSubperiodo(props) {
	const router = useRouter();
	const searchParams = useSearchParams();
	...
	const paramPeriodoId = React.useMemo(
		() => searchParams.get(periodoParams.subPeriodos),
		[searchParams],
	);
	...
	return (
		...
		>
			<DialogContent className='max-h-[80%] max-w-xs overflow-y-scroll sm:max-w-[425px] md:max-w-2xl'>
				<DialogHeader>
					<DialogTitle>Editar Subperiodo</DialogTitle>
					<Form {...form}>
						{fieldsSubPeriodos.map(f => (
							<FormField
								control={form.control}
								name={f.name}
								key={f.name.includes("Desde") ? f.name + form.watch() : f.name}
