...
function UpdateRequisito(props) {
	...
	const paramPeriodoId = React.useMemo(
		() => searchParams.get(reqParams.update),
		[searchParams],
	);
	...
	if (!selectedPeriodo) {
		return (
			<ModalFallback
				action='update'
				redirectTo={() => router.replace(ROUTES.periodo.requisito(props.id))}
			/>
		);
	}
	return (
		<Dialog
			...
			onOpenChange={open => {
				...
				if (!open) {
					router.replace(ROUTES.periodo.requisito(props.id));
					return;
				}
			}}
		>
			<DialogContent className='max-h-[80%] max-w-xs overflow-y-scroll sm:max-w-[425px] md:max-w-2xl'>
				<DialogHeader>
					<DialogTitle>Actualizar cronograma de matriculación</DialogTitle>
				</DialogHeader>
				<Form {...form}>
					<form
